...
import { 
  Tv, Filter, ChevronDown, Plus, RefreshCw, Search, 
  FolderOpen, Play, Globe, HardDrive, Clock, Star, Layers
} from 'lucide-react';
...
export const TVShowsPage = () => {
  ...
  const fetchLibraryData = useCallback(async () => {
    try {
      const [librariesRes, mediaRes, seriesRes] = await Promise.all([
        marmaladeLibrary.getLibraries(),
        marmaladeMedia.getMedia({ media_type: 'episode', limit: 500 }),
        marmaladeMedia.getTVSeriesGrouped(),
      ]);
      const tvLibraries = (librariesRes.data || []).filter(lib => 
        lib.media_type === 'tv' || lib.media_type === 'tv_shows'
      );
      setLibraries(tvLibraries);
      setLocalEpisodes(mediaRes.data || []);
  ...
  const handleAddLibrary = async () => {
    ...
    try {
      const addRes = await marmaladeLibrary.addLibrary(newLibrary.name, newLibrary.path, 'tv');
      toast.success(`TV library "${newLibrary.name}" added`);
  ...
  // Group episodes by series for display
  const seriesMap = {};
  localEpisodes.forEach(ep => {
    const seriesKey = ep.series_name || ep.title || 'Unknown Series';
    ...
    seriesMap[seriesKey].episodes.push(ep);
  });
  const seriesList = Object.values(seriesMap).filter(s =>
  ...
  const fetchShows = async () => {
    ...
    try {
      ...
      if (page === 1) setShows(response.data.results || []);
      else setShows(prev => [...prev, ...(response.data.results || [])]);
      ...
      <div data-testid="tv-shows-page" className="min-h-screen p-8">
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400">
                  {viewMode === 'library'
                    ? `${seriesList.length} series, ${localEpisodes.length} episodes in ${libraries.length} ${libraries.length === 1 ? 'library' : 'libraries'}`
                    : 'Explore series and episodes'}
                </p>
              </div>
            </div>

            <div className="flex items-center gap-3">
              <div className="flex rounded-lg bg-white/5 p-1" data-testid="tv-view-toggle">
                <button
                  onClick={() => setViewMode('library')}
                  data-testid="tv-library-tab"
                  className={`px-4 py-2 rounded-md flex items-center gap-2 transition-colors text-sm font-medium ${
                    viewMode === 'library' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'
                  }`}
                >
                ...
                <button
                  ...
                  data-testid="tv-discover-tab"
                  className={`px-4 py-2 rounded-md flex items-center gap-2 transition-colors text-sm font-medium ${
                    viewMode === 'discover' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'
                  }`}
                >
              ...
              {viewMode === 'library' && (
                <Button
                  ...
                  data-testid="add-tv-library-btn"
                >
          ...
          {viewMode === 'library' && showAddLibrary && (
            ...
            >
              <h3 className="font-semibold mb-3">Add TV Show Library</h3>
              <div className="flex gap-3">
                <Input
                  placeholder="Library Name (e.g., My TV Shows)"
                  value={newLibrary.name}
                  onChange={(e) => setNewLibrary(prev => ({ ...prev, name: e.target.value }))}
                  className="bg-white/5 border-white/10"
                  data-testid="tv-library-name-input"
                />
                <Input
                  placeholder="Path (e.g., /home/user/TV Shows)"
                  value={newLibrary.path}
                  onChange={(e) => setNewLibrary(prev => ({ ...prev, path: e.target.value }))}
                  className="bg-white/5 border-white/10 flex-1"
                  data-testid="tv-library-path-input"
                />
                <Button onClick={handleAddLibrary} className="bg-green-600 hover:bg-green-700" data-testid="tv-library-add-btn">
                  Add & Scan
                </Button>
              </div>
            </motion.div>
          ...
          <>
            ...
            {libraries.length === 0 && !libraryLoading && (
              ...
              >
                ...
                <h2 className="text-xl font-bold mb-2">No TV Show Libraries</h2>
                <p className="text-gray-400 text-center max-w-md mb-6">
                  Add a TV show library to start viewing your collection.
                  Organize shows by folder: /Show Name/Season 01/S01E01.mkv
                </p>
                <div className="flex gap-3">
                  <Button
            ...
            {libraries.length > 0 && (
              <div className="mb-8">
                <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
                  <FolderOpen className="w-5 h-5 text-blue-400" /> Your Libraries
                </h2>
            ...
            {seriesList.length > 0 && (
              <div>
                <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
                  <Layers className="w-5 h-5 text-blue-400" /> All Series ({seriesList.length})
                </h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                  {seriesList.map((series, index) => (
                    <Link key={series.name} to={series.tmdb_id ? `/tv/${series.tmdb_id}` : '#'} data-testid={`local-series-${index}`}>
                      <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                      ...
                      >
                        <div className="aspect-[2/3] relative">
                          {series.poster_url ? (
                            ...
                            <div className="w-full h-full bg-gradient-to-br from-blue-600/30 to-cyan-500/30 flex items-center justify-center">
                              <Tv className="w-10 h-10 text-blue-400" />
                            </div>
                          )}
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3">
                            <span className="text-xs bg-blue-600/80 px-2 py-0.5 rounded-full">
                ...
                <DropdownMenuContent className="bg-[#1E1E1E] border-white/10">
                  {sortOptions.map((option) => (
                    <DropdownMenuItem
                      ...
                      className={sortBy === option.value ? 'bg-blue-600/20 text-blue-400' : ''}
                    >
                      {option.label}
                    </DropdownMenuItem>
                  ))}
