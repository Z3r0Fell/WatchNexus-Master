...
const BiscottiPage = () => {
  ...
  const fetchItems = useCallback(async () => {
    ...
    } catch { console.error('[BiscottiPage] Failed to fetch items'); } finally { setLoading(false); }
  }, [typeFilter, searchQuery]);

  const fetchStats = useCallback(async () => {
    try { const res = await axios.get(`${API}/api/biscotti/stats`); setStats(res.data); } catch { console.error('[BiscottiPage] Failed to fetch stats'); }
  }, []);
