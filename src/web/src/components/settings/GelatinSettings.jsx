...
export const GelatinSettings = () => {
  ...
  const fetchGelatinStatus = useCallback(async () => {
    try { const res = await axios.get(`${BACKEND_URL}/api/gelatin/status`); setGelatinStatus(res.data); } catch { console.error('[GelatinSettings] Failed to fetch status'); }
  }, []);

  const fetchActiveTunnels = useCallback(async () => {
    try { const res = await axios.get(`${BACKEND_URL}/api/gelatin/tunnels`); setActiveTunnels(res.data || []); } catch { console.error('[GelatinSettings] Failed to fetch tunnels'); }
  }, []);
