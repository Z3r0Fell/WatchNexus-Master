...
if (!isTesting)
{
    ...
    bootLogPath = Path.Combine(logDir, $"boot-{DateTime.UtcNow:yyyyMMdd-HHmmss}.log");
    bootLog = new StreamWriter(bootLogPath, append: false) { AutoFlush = true };
...
else
{
    ...
    Log = msg => Console.WriteLine($"[TEST] {msg}");
}

Log($"[WatchNexus] Boot log: {logDir ?? "console only"}");
...
// ══════════════════════════════════════════════════════════════════════
if (!isTesting && (args.Contains("--tray") || args.Contains("--tray-only")))
{
...
// In test environment, skip file system operations that don't work in tests
if (isTesting)
{
    // Test environment will configure its own database via WebApplicationFactory
}
...
{
    // Test environment: in-memory database already configured above
    builder.Services.AddDataProtection()
...
var jwtSecret = ResolveJwtSecret(builder.Configuration, dataDir, Log, isTesting);
...
if (!isTesting && string.IsNullOrWhiteSpace(builder.Configuration["TMDB_API_KEY"]))
...
if (!isTesting && string.IsNullOrWhiteSpace(builder.Configuration["LICENSE_SERVER_API_KEY"]))
...
//
// Disabled under the test host: TestServer requests share a single
// null RemoteIpAddress ("unknown") partition, so the per-IP limiter
// would 429 unrelated integration tests after a handful of requests.
...
static string ResolveJwtSecret(IConfiguration config, string dataDir, Action<string> log, bool isTesting)
{
    ...
    if (isTesting)
    {
        // Tests run against a deterministic-but-strong secret; never touch disk.
...
// ── Global Exception Handler — must be early to catch all downstream errors
...
// Explicit entry-point hook for Microsoft.AspNetCore.Mvc.Testing
// (WebApplicationFactory<Program>). Top-level statements generate an
// internal Program by default; making it public + partial lets the
// test factory defer host building instead of launching the real server.
