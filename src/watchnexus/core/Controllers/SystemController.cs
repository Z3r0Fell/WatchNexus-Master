...
public class DbControllerReal : ControllerBase
{
    ...
    public IActionResult Roadmap()
    {
        ...
        return Ok(new { version = "1.0.4", endpoints = items, total = items.Length });
    }
    ...
    public IActionResult Changelog()
    {
        ...
        if (!System.IO.File.Exists(changelogPath))
            return Ok(new { version = "1.0.4", content = "# WatchNexus Changelog\n\nNo changelog available." });
        var content = System.IO.File.ReadAllText(changelogPath);
        return Ok(new { version = "1.0.4", content = content });
    }
}
