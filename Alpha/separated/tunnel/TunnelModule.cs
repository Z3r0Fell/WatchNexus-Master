...
public class VpnController : ControllerBase
{
    ...
    public VpnController(TunnelDbContext db) { _db = db; }
    ...
    public async Task<IActionResult> GetServer()
    {
        ...
        return Ok(new { s.ListenPort, s.Address, s.Dns, s.Endpoint, s.Mtu,
            public_key = s.PublicKey, is_active = s.IsActive, is_configured = s.IsConfigured, @interface = "wg0" });
    }
    ...
    [HttpGet("peers/{id}")]
    public async Task<IActionResult> GetPeer(string id) { var p = await _db.VpnPeers.FindAsync(id); return p == null ? NotFound() : Ok(p); }
    ...
    public async Task<IActionResult> CreatePeer([FromBody] PeerCreate req)
    {
        ...
        {
            Name = req.Name, AllowedIps = req.AllowedIps,
    ...
    {
        var p = await _db.VpnPeers.FindAsync(id); if (p == null) return NotFound();
    ...
    {
        var p = await _db.VpnPeers.FindAsync(id); if (p == null) return NotFound();
        _db.VpnPeers.Remove(p); await _db.SaveChangesAsync();
    ...
    {
        var p = await _db.VpnPeers.FindAsync(id); if (p == null) return NotFound();
        p.IsActive = !p.IsActive; await _db.SaveChangesAsync(); return Ok(p);
    }
    ...
    public IActionResult WgUp() => Ok(new { status = "up", message = "WireGuard activated" });
    [HttpPost("server/wg-down")]
    public IActionResult WgDown() => Ok(new { status = "down", message = "WireGuard deactivated" });
    ...
    {
        var s = await GetOrCreateServer();
        return Ok(new { server_active = s.IsActive, server_configured = s.IsConfigured,
            total_peers = await _db.VpnPeers.CountAsync(), active_peers = await _db.VpnPeers.CountAsync(p => p.IsActive),
            total_rx = await _db.VpnPeers.SumAsync(p => p.TransferRx), total_tx = await _db.VpnPeers.SumAsync(p => p.TransferTx) });
    }
}

// ── Module Registration ──────────────────────────────────────
public class TunnelModule : IWatchNexusModule
{
    public ModuleManifest Manifest => new()
    {
        Name = "Tunnel", Codename = "tunnel",
        DisplayName = "VPN Portal", Version = "2.7.3-alpha",
