...
/**
 * Utility class used to collect information about images in WatchNexus API responses to easily pass around the app.
 */
data class WatchNexusImage(
	val item: UUID,
	val source: WatchNexusImageSource,
	val type: ImageType,
	val tag: String,
	val blurHash: String?,
...
fun WatchNexusImage.getUrl(
	...
	WatchNexusImageSource.USER -> api.imageApi.getUserImageUrl(
		userId = item,
		tag = tag,
	)
...
enum class WatchNexusImageSource {
	ITEM,
	PARENT,
	ALBUM,
	SERIES,
...
val UserDto.primaryImage
	get() = primaryImageTag?.let { primaryImageTag ->
		WatchNexusImage(
			item = id,
			source = WatchNexusImageSource.USER,
...
val BaseItemDto.itemImages
	get() = imageTags?.mapValues { (type, tag) ->
		WatchNexusImage(
			item = id,
			source = WatchNexusImageSource.ITEM,
			type = type,
			tag = tag,
	...
	get() = mapOf(
		...
		itemAndTag.first?.let { item ->
			itemAndTag.second?.let { tag ->
				WatchNexusImage(
					item = item,
					source = WatchNexusImageSource.PARENT,
					type = type,
					tag = tag,
					blurHash = imageBlurHashes?.get(type)?.get(tag),
					aspectRatio = null,
...
val BaseItemDto.albumPrimaryImage
	get() = albumPrimaryImageTag?.let { albumPrimaryImageTag ->
		albumId?.let { albumId ->
			WatchNexusImage(
				item = albumId,
				source = WatchNexusImageSource.ALBUM,
				type = ImageType.PRIMARY,
...
val BaseItemDto.channelPrimaryImage
	get() = channelPrimaryImageTag?.let { channelPrimaryImageTag ->
		channelId?.let { channelId ->
			WatchNexusImage(
				item = channelId,
				source = WatchNexusImageSource.CHANNEL,
				type = ImageType.PRIMARY,
...
val BaseItemDto.seriesPrimaryImage
	get() = seriesPrimaryImageTag?.let { seriesPrimaryImageTag ->
		seriesId?.let { seriesId ->
			WatchNexusImage(
				item = seriesId,
				source = WatchNexusImageSource.SERIES,
				type = ImageType.PRIMARY,
...
val BaseItemDto.chapterImages
	get() = chapters?.mapIndexed { index, chapter ->
		WatchNexusImage(
			item = id,
			source = WatchNexusImageSource.CHAPTER,
