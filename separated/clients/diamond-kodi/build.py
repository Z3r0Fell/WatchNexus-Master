...
def zip_files(py_version: str, source: str, target: str, dev: bool) -> None:
    ...
    """
    archive_name = "plugin.video.watchnexus+{}.zip".format(py_version)
            ...
            for filename in filter(file_filter, files):
                ...
                if dev or folder_filter(file_path):
                    relative_path = os.path.join(
                        "plugin.video.watchnexus", os.path.relpath(file_path, source)
                    )
...
def file_filter(file_name: str) -> bool:
    ...
    return (
        not (
            file_name.startswith("plugin.video.watchnexus") and file_name.endswith(".zip")
        )
