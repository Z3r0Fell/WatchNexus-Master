...
class Movies(KodiDb):
    ...
    def __init__(self, server, watchnexusdb, videodb, direct_path, library=None):
    ...
    def movie_add(self, obj):
        ...
        self.watchnexus_db.add_reference(*values(obj, QUEM.add_reference_movie_obj))
        LOG.debug(
            "ADD movie [%s/%s/%s] %s: %s",
            obj["PathId"],
            obj["FileId"],
    ...
    def trailer(self, obj):
        ...
        try:
            if obj["LocalTrailer"]:
                ...
                trailer = self.server.watchnexus.get_local_trailers(obj["Id"])
                obj["Trailer"] = (
                    "plugin://plugin.video.watchnexus/trailer?id=%s&mode=play"
                    % trailer[0]["Id"]
                )
    ...
    def get_path_filename(self, obj):
        ...
        if self.direct_path:
            ...
            obj["Path"] = "plugin://plugin.video.watchnexus/%s/" % obj["LibraryId"]
            params = {
    ...
    def boxset(self, item, e_item):
        ...
        self.watchnexus_db.add_reference(*values(obj, QUEM.add_reference_boxset_obj))
    ...
    def boxset_current(self, obj):
        ...
        try:
            current = self.watchnexus_db.get_item_id_by_parent_id(
        ...
        for all_movies in server.get_movies_by_boxset(obj["Id"]):
            for movie in all_movies["Items"]:
                ...
                try:
                    temp_obj["MovieId"] = self.watchnexus_db.get_item_by_id(
    ...
    def boxsets_reset(self):
        ...
        boxsets = self.watchnexus_db.get_items_by_media("set")
    ...
    def remove(self, item_id, e_item):
        """Remove movieid, fileid, watchnexus reference.
        ...
        if obj["Media"] == "movie":
            ...
            for movie in self.watchnexus_db.get_item_by_parent_id(
