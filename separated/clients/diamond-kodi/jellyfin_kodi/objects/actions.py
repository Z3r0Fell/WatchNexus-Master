...
class Actions(object):
    ...
    def __init__(self, server_id=None, api_client=None):
        ...
        if not api_client:
            ...
            watchnexus_client = WatchNexus(server_id).get_client()
    ...
    def _set_intros(self, item):
        ...
        if intros["Items"]:
            ...
            if settings("askCinema") == "true":
                ...
                resp = dialog("yesno", "{watchnexus}", translate(33016))
                if not resp:
            ...
            if enabled:
                ...
                window("watchnexus.skip.%s" % intro["Id"], value="true")
        ...
        if not intro and obj["Type"] != "Trailer":
            ...
            obj["Artwork"]["Primary"] = (
                ...
                or (
                    ...
                    else "special://home/addons/plugin.video.watchnexus/resources/fanart.png"
                )
            )
            obj["Artwork"]["Primary"] += (
                "&KodiTrailer=true"
    ...
    def listitem_channel(self, obj, listitem, item):
        ...
        obj["Artwork"]["Backdrop"] = obj["Artwork"]["Backdrop"] or [
            "special://home/addons/plugin.video.watchnexus/resources/fanart.png"
        ]
    ...
    def resume_dialog(self, seektime):
        ...
        XML_PATH = (
            xbmcaddon.Addon("plugin.video.watchnexus").getAddonInfo("path"),
            "default",
            "1080i",
        )

        dialog = resume.ResumeDialog("script-watchnexus-resume.xml", *XML_PATH)
...
def on_update(data, server):
    ...
    if item:
        ...
        if not window("watchnexus.skip.%s.bool" % item[0]):
            server.watchnexus.item_played(item[0], playcount)

        window("watchnexus.skip.%s" % item[0], clear=True)


def on_play(data, server):
    """Setup progress for watchnexus playback."""
    player = xbmc.Player()
    ...
    if settings("useDirectPaths") == "1" or media == "song":
        ...
        if item:
            ...
            item = server.watchnexus.get_item(item[0])
...
def special_listener():
    ...
    count = int(window("watchnexus.external_count") or 0)

    if is_playing and not window("watchnexus.external_check"):
        time = player.getTime()

        if time > 1:  # Not external player.

            window("watchnexus.external_check", value="true")
            window("watchnexus.external_count", value="0")
        elif count == 120:

            LOG.info("External player detected.")
            window("watchnexus.external.bool", True)
            window("watchnexus.external_check.bool", True)
            window("watchnexus.external_count", value="0")

        elif time == 0:
            window("watchnexus.external_count", value=str(count + 1))
