...
from ..database import watchnexus_db, queries as QUEM
from ..helper import (
    api,
    stop,
    validate,
...
class TVShows(KodiDb):
    ...
    def __init__(
        ...
        self.server = server
        self.watchnexus = watchnexusdb
        self.video = videodb
        self.direct_path = direct_path
        self.update_library = update_library

        self.watchnexus_db = watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor)
    ...
    def tvshow(self, item, e_item):
        ...
        try:
            ...
            obj["LibraryName"] = self.watchnexus_db.get_view_name(obj["LibraryId"])
        except TypeError:
            update = False
        ...
        for season in self.server.watchnexus.get_seasons(obj["Id"])["Items"]:
            ...
            if season["SeriesId"] != obj["Id"]:
                ...
                try:
                    self.watchnexus_db.get_item_by_id(
                    ...
                    self.watchnexus_db.add_reference(
            ...
            try:
                self.watchnexus_db.get_item_by_id(season["Id"])[0]
    ...
    def tvshow_add(self, obj):
        ...
        if self.direct_path:
            ...
            temp_obj = dict()
            temp_obj["TopLevel"] = "plugin://plugin.video.watchnexus/"
        ...
        self.watchnexus_db.add_reference(*values(obj, QUEM.add_reference_tvshow_obj))
        LOG.debug(
    ...
    def tvshow_update(self, obj):
        ...
        self.watchnexus_db.update_reference(*values(obj, QUEM.update_reference_obj))
        LOG.debug(
    ...
    def get_path_filename(self, obj):
        ...
        if self.direct_path:
            ...
            if "\\" in obj["Path"]:
                ...
                obj["Path"] = "%s/" % obj["Path"]
                obj["TopLevel"] = "plugin://plugin.video.watchnexus/"

            if not validate(obj["Path"]):
                raise PathValidationException("Failed to validate path. User stopped.")
        else:
            obj["TopLevel"] = "plugin://plugin.video.watchnexus/%s/" % obj["LibraryId"]
    ...
    def season(self, item, show_id=None):
        ...
        if obj["ShowId"] is None:
            ...
            try:
                obj["ShowId"] = self.watchnexus_db.get_item_by_id(
                    *values(obj, QUEM.get_item_series_obj)
                )[0]
        ...
        if obj["Location"] != "Virtual":
            self.watchnexus_db.add_reference(*values(obj, QUEM.add_reference_season_obj))
            self.item_ids.append(obj["Id"])
    ...
    def episode(self, item, e_item):
        ...
        if not self.direct_path and obj["Resume"]:
            ...
            temp_obj = dict(obj)
            temp_obj["Path"] = "plugin://plugin.video.watchnexus/"
    ...
    def episode_add(self, obj):
        ...
        self.watchnexus_db.add_reference(*values(obj, QUEM.add_reference_episode_obj))

        parentPathId = self.watchnexus_db.get_episode_kodi_parent_path_id(
    ...
    def episode_update(self, obj):
        ...
        self.watchnexus_db.update_reference(*values(obj, QUEM.update_reference_obj))
        self.watchnexus_db.update_parent_id(*values(obj, QUEM.update_parent_episode_obj))
        LOG.debug(
    ...
    def get_episode_path_filename(self, obj):
        ...
        if self.direct_path:
            ...
            obj["LibraryId"] = library["Id"]
            obj["Path"] = "plugin://plugin.video.watchnexus/%s/%s/" % (
                obj["LibraryId"],
                obj["SeriesId"],
            )
            params = {
    ...
    def get_show_id(self, obj):
        ...
        if obj["ShowId"] is None:
            ...
            try:
                self.tvshow(self.server.watchnexus.get_item(obj["SeriesId"]))
                obj["ShowId"] = self.watchnexus_db.get_item_by_id(
                    *values(obj, QUEM.get_item_series_obj)
                )[0]
    ...
    def userdata(self, item, e_item):
        ...
        if obj["Media"] == "tvshow":
            ...
            if not self.direct_path and not obj["Resume"]:
                ...
                temp_obj["PathId"] = self.get_path("plugin://plugin.video.watchnexus/")
                temp_obj["FileId"] = self.add_file(*values(temp_obj, QU.add_file_obj))
                self.update_file(*values(temp_obj, QU.update_file_obj))
                self.add_playstate(*values(temp_obj, QU.add_bookmark_obj))
    ...
    def remove(self, item_id, e_item):
        """Remove showid, fileid, pathid, watchnexus reference.
        ...
        if obj["Media"] == "episode":
            ...
            season = self.watchnexus_db.get_full_item_by_kodi_id(
            ...
            if not self.watchnexus_db.get_item_by_parent_id(
                *values(obj, QUEM.get_item_by_parent_episode_obj)
            ):

                self.remove_season(obj["ParentId"], obj["Id"])
                self.watchnexus_db.remove_item(*values(temp_obj, QUEM.delete_item_obj))

            temp_obj["Id"] = self.watchnexus_db.get_item_by_kodi_id(
            ...
            if not self.get_total_episodes(
                *values(temp_obj, QU.get_total_episodes_obj)
                ...
                for season in self.watchnexus_db.get_item_by_parent_id(
                    *values(temp_obj, QUEM.get_item_by_parent_season_obj)
                ):
                    self.remove_season(season[1], obj["Id"])
                else:
                    self.watchnexus_db.remove_items_by_parent_id(
            ...
            for season in self.watchnexus_db.get_item_by_parent_id(
                *values(obj, QUEM.get_item_by_parent_season_obj)
                ...
                for episode in self.watchnexus_db.get_item_by_parent_id(
            ...
            if not self.watchnexus_db.get_item_by_parent_id(
                *values(obj, QUEM.delete_item_by_parent_season_obj)
                ...
                self.watchnexus_db.remove_item_by_kodi_id(
                    *values(obj, QUEM.delete_item_by_parent_tvshow_obj)
                )

        # Remove any series pooling episodes
        for episode in self.watchnexus_db.get_media_by_parent_id(obj["Id"]):
            self.remove_episode(episode[2], episode[3], obj["Id"])
        else:
            self.watchnexus_db.remove_media_by_parent_id(obj["Id"])

        self.watchnexus_db.remove_item(*values(obj, QUEM.delete_item_obj))
    ...
    def get_child(self, item_id, e_item):
        """Get all child elements from tv show watchnexus id."""
        obj = {"Id": item_id}
        child = []
