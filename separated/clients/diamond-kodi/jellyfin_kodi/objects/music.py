...
from ..helper import api, stop, validate, watchnexus_item, values, Local, LazyLogger
...
class Music(KodiDb):
    ...
    def __init__(self, server, watchnexusdb, musicdb, direct_path, library=None):
    ...
    def artist_add(self, obj):
        ...
        safety checks: It looks like WatchNexus supports the same artist multiple times.
        Kodi doesn't allow that. In case that happens we just merge the artist entries.
        """
        obj["ArtistId"] = self.get(*values(obj, QU.get_artist_obj))
        self.watchnexus_db.add_reference(*values(obj, QUEM.add_reference_artist_obj))
    ...
    def album_add(self, obj):
        ...
        self.watchnexus_db.add_reference(*values(obj, QUEM.add_reference_album_obj))
    ...
    def artist_discography(self, obj):
        ...
        for artist in obj["ArtistItems"] or []:
            ...
            try:
                temp_obj["ArtistId"] = self.watchnexus_db.get_item_by_id(
            ...
            self.watchnexus_db.update_parent_id(
                *values(temp_obj, QUEM.update_parent_album_obj)
            )

    def artist_link(self, obj):
        """Assign main artists to album.
        Artist does not exist in watchnexus database, create the reference.
        """
        for artist in obj["AlbumArtists"] or []:
            ...
            try:
                ...
                try:
                    self.artist(self.server.watchnexus.get_item(temp_obj["Id"]))
                    temp_obj["ArtistId"] = self.watchnexus_db.get_item_by_id(
                        *values(temp_obj, QUEM.get_item_obj)
                    )[0]
    ...
    def song_add(self, obj):
        ...
        try:
            obj["AlbumId"] = self.watchnexus_db.get_item_by_id(
            ...
            try:
                ...
                self.album(self.server.watchnexus.get_item(obj["SongAlbumId"]))
                obj["AlbumId"] = self.watchnexus_db.get_item_by_id(
                    *values(obj, QUEM.get_item_song_obj)
                )[0]
            except TypeError:
        ...
        self.watchnexus_db.add_reference(*values(obj, QUEM.add_reference_song_obj))
        LOG.debug(
            "ADD song [%s/%s/%s] %s: %s",
            obj["PathId"],
            obj["AlbumId"],
    ...
    def remove(self, item_id, e_item):
        ...
        if obj["Media"] == "song":
            ...
            self.watchnexus_db.remove_wild_item(obj["Id"])

            for item in self.watchnexus_db.get_item_by_wild_id(
            ...
            for song in self.watchnexus_db.get_item_by_parent_id(
            ...
            for album in self.watchnexus_db.get_item_by_parent_id(
