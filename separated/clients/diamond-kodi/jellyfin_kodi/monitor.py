...
class Monitor(xbmc.Monitor):
    ...
    def onNotification(self, sender, method, data):
        ...
        if sender == "plugin.video.watchnexus":
            ...
            if method not in ("plugin.video.watchnexus_play_action",):
            ...
            if not data.get("ServerId"):
                ...
                if method != "LoadServer" and data["ServerId"] not in self.servers:
                    ...
                    try:
                        ...
                        LOG.exception(error)
                        dialog("ok", "{watchnexus}", translate(33142))

                        return

                server = WatchNexus(data["ServerId"])
        except Exception as error:
            LOG.exception(error)
            server = WatchNexus()
        ...
        if method == "Play":
            ...
            items = server.watchnexus.get_items(data["ItemIds"])
        ...
        # TODO no clue if this is called by anything
            ...
            server.watchnexus.post_session(
            ...
            server.watchnexus.session_add_user(
    ...
    def server_instance(self, server_id=None):
        ...
        server = WatchNexus(server_id).get_client()
        session = server.watchnexus.get_device(self.device_id)
        ...
        if server_id is not None:
            ...
            all_users = server.watchnexus.get_users()
    ...
    def additional_users(self, server):
        ...
        for i in range(10):
            window("WatchNexusAdditionalUserImage.%s" % i, clear=True)
        ...
        for index, user in enumerate(session[0]["AdditionalUsers"]):
            ...
            info = server.watchnexus.get_user(user["UserId"])
            image = api.API(info, server.config.data["auth.server"]).get_user_artwork(
                user["UserId"]
            )
            window("WatchNexusAdditionalUserImage.%s" % index, image)
            window("WatchNexusAdditionalUserPosition.%s" % user["UserId"], str(index))

    def playstate(self, data):
        """WatchNexus playstate updates."""
    ...
    def general_commands(self, data):
        """General commands from WatchNexus to control the Kodi interface."""
        command = data["Name"]
        args = data["Arguments"]
