...
class Context(object):
    ...
    def __init__(self, transcode=False, delete=False):
        ...
        try:
            ...
            self.server_id = sys.listitem.getProperty("watchnexusserver") or None
            self.api_client = WatchNexus(self.server_id).get_client().watchnexus
            item_id = sys.listitem.getProperty("watchnexusid")
        except AttributeError:
            self.server_id = None

            if xbmc.getInfoLabel("ListItem.Property(watchnexusid)"):
                item_id = xbmc.getInfoLabel("ListItem.Property(watchnexusid)")
            else:
    ...
    def get_item_id(self):
        """Get synced item from watchnexusdb."""
    ...
    def select_menu(self):
        ...
        context_menu = context.ContextMenu("script-watchnexus-context.xml", *XML_PATH)
    ...
    def delete_item(self):
        if settings("skipContextMenu.bool") or dialog(
            "yesno", "{watchnexus}", translate(33015)
        ):
