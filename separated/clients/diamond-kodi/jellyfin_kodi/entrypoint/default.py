...
from .. import client
from ..database import reset, get_sync, Database, watchnexus_db, get_credentials
...
class Events(object):
    ...
    def __init__(self):
        ...
        watchnexus_client = WatchNexus(server).get_client()
        api_client = watchnexus_client.watchnexus

        addon_data = translate_path(
            "special://profile/addon_data/plugin.video.watchnexus/data.json"
        )
        try:
        ...
        if "/extrafanart" in base_url:
            ...
            watchnexus_path = path[1:]
            watchnexus_id = params.get("id")
            get_fanart(watchnexus_id, watchnexus_path, server, api_client)

        elif "/Extras" in base_url or "/VideoFiles" in base_url:

            watchnexus_path = path[1:]
            watchnexus_id = params.get("id")
            get_video_extras(watchnexus_id, watchnexus_path, server, api_client)
            ...
            xbmc.executebuiltin("Addon.OpenSettings(plugin.video.watchnexus)")
        elif mode == "adduser":
            add_user(api_client)
            ...
            window("watchnexus.restart.bool", True)
        elif (
            mode is None
            and not params
            and base_url != "plugin://plugin.video.watchnexus/"
        ):
...
def listing():
    """Display all watchnexus nodes and dynamic entries when appropriate."""
    total = int(window("WatchNexus.nodes.total") or 0)
    sync = get_sync()
    ...
    for i in range(total):
        ...
        window_prop = "WatchNexus.nodes.%s" % i
        ...
        if (
            ...
            context.append(
                (
                    translate(33123),
                    "RunPlugin(plugin://plugin.video.watchnexus/?mode=synclib&id=%s)"
                    % view_id,
                )
            )
        ...
        if (
            ...
            context.append(
                (
                    translate(33136),
                    "RunPlugin(plugin://plugin.video.watchnexus/?mode=updatelib&id=%s)"
                    % view_id,
                )
            )
            context.append(
                (
                    translate(33132),
                    "RunPlugin(plugin://plugin.video.watchnexus/?mode=repairlib&id=%s)"
                    % view_id,
                )
            )
            context.append(
                (
                    translate(33133),
                    "RunPlugin(plugin://plugin.video.watchnexus/?mode=removelib&id=%s)"
                    % view_id,
                )
            )
    ...
    for server in servers:
        ...
        if server.get("ManualAddress"):
            context.append(
                (
                    translate(33141),
                    "RunPlugin(plugin://plugin.video.watchnexus/?mode=removeserver&server=%s)"
                    % server["Id"],
                )
            )
        ...
        if "AccessToken" not in server:
            directory(
                ...
                "plugin://plugin.video.watchnexus/?mode=login&server=%s" % server["Id"],
                False,
                context=context,
            )
        else:
            directory(
                server["Name"],
                "plugin://plugin.video.watchnexus/?mode=browse&server=%s" % server["Id"],
                context=context,
            )

    directory(translate(33194), "plugin://plugin.video.watchnexus/?mode=managelibs", True)
    directory(
        translate(33203), "plugin://plugin.video.watchnexus/?mode=managepaths", True
    )
    directory(translate(33134), "plugin://plugin.video.watchnexus/?mode=addserver", False)
    directory(translate(33054), "plugin://plugin.video.watchnexus/?mode=adduser", False)
    directory(translate(5), "plugin://plugin.video.watchnexus/?mode=settings", False)
    directory(
        translate(33161), "plugin://plugin.video.watchnexus/?mode=updatepassword", False
    )
    directory(translate(33058), "plugin://plugin.video.watchnexus/?mode=reset", False)
    directory(
        translate(33180), "plugin://plugin.video.watchnexus/?mode=restartservice", False
    )

    if settings("backupPath"):
        directory(
            translate(33092), "plugin://plugin.video.watchnexus/?mode=backup", False
        )
...
def dir_listitem(label, path, artwork=None, fanart=None):
    ...
    li.setArt(
        {
            "thumb": artwork
            or "special://home/addons/plugin.video.watchnexus/resources/icon.png",
            "fanart": fanart
            or "special://home/addons/plugin.video.watchnexus/resources/fanart.png",
            "landscape": artwork
            or fanart
            or "special://home/addons/plugin.video.watchnexus/resources/fanart.png",
        }
...
def manage_libraries():
    ...
    directory(
        translate(33098), "plugin://plugin.video.watchnexus/?mode=refreshboxsets", False
    )
    directory(translate(33154), "plugin://plugin.video.watchnexus/?mode=addlibs", False)
    directory(
        translate(33139), "plugin://plugin.video.watchnexus/?mode=updatelibs", False
    )
    directory(
        translate(33140), "plugin://plugin.video.watchnexus/?mode=repairlibs", False
    )
    directory(
        translate(33184), "plugin://plugin.video.watchnexus/?mode=removelibs", False
    )
    directory(
        translate(33060), "plugin://plugin.video.watchnexus/?mode=thememedia", False
    )
...
def browse(media, view_id=None, folder=None, server_id=None, api_client=None):
    ...
    if not window("watchnexus_online.bool") and server_id is None:
    ...
    if listing:
        ...
        for item in listing:
            ...
            li = xbmcgui.ListItem()
            li.setProperty("watchnexusid", item["Id"])
            li.setProperty("watchnexusserver", server_id)
            ...
            if item.get("IsFolder"):
                ...
                path = "%s?%s" % ("plugin://plugin.video.watchnexus/", urlencode(params))
                context = []
                ...
                if item["Type"] in ("Series", "Season", "Playlist"):
                    context.append(
                        (
                            "Play",
                            "RunPlugin(plugin://plugin.video.watchnexus/?mode=playlist&id=%s&server=%s)"
                            % (item["Id"], server_id),
                        )
                    )
                ...
                if item["UserData"]["Played"]:
                    context.append(
                        (
                            translate(16104),
                            "RunPlugin(plugin://plugin.video.watchnexus/?mode=unwatched&id=%s&server=%s)"
                            % (item["Id"], server_id),
                        )
                    )
                else:
                    context.append(
                        (
                            translate(16103),
                            "RunPlugin(plugin://plugin.video.watchnexus/?mode=watched&id=%s&server=%s)"
                            % (item["Id"], server_id),
                        )
                    )
                ...
                if item["Type"] not in ("Photo", "PhotoAlbum"):
                    ...
                    path = "%s?%s" % (
                        "plugin://plugin.video.watchnexus/",
                        urlencode(params),
                    )
...
def browse_subfolders(media, view_id, server_id=None):
    """Display submenus for watchnexus views."""
    from ..views import DYNNODES

    view = WatchNexus(server_id).get_client().watchnexus.get_item(view_id)
    xbmcplugin.setPluginCategory(PROCESS_HANDLE, view["Name"])
...
def get_fanart(item_id, path, server_id=None, api_client=None):
    ...
    """
    if not item_id and "plugin.video.watchnexus" in path:
    ...
    objects = Objects()
    list_li = []
    directory = translate_path("special://thumbnails/watchnexus/%s/" % item_id)
...
def get_video_extras(item_id, path, server_id=None, api_client=None):
    ...
    # TODO implement????
    # WatchNexus(server_id).get_client().watchnexus.get_item(item_id)

    """
    def getVideoFiles(watchnexusId,watchnexusPath):
        #returns the video files for the item as plugin listing, can be used for browsing the actual files or videoextras etc.
        watchnexus = watchnexusserver.Read_WatchNexusServer()
        if not watchnexusId:
            if "plugin.video.watchnexus" in watchnexusPath:
                watchnexusId = watchnexusPath.split("/")[-2]
        if watchnexusId:
            item = watchnexus.getItem(watchnexusId)
...
def get_next_episodes(item_id, limit):
    ...
    with Database("watchnexus") as watchnexusdb:

        db = watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor)
...
def get_themes(api_client):
    ...
    library = translate_path(
        "special://profile/addon_data/plugin.video.watchnexus/library"
    )
    ...
    if xbmc.getCondVisibility("System.HasAddon(script.tvtunes)"):
        ...
        dialog("ok", "{watchnexus}", translate(33152))

        return

    with Database("watchnexus") as watchnexusdb:
        all_views = watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor).get_views()
        views = [
            x.view_id
            for x in all_views
...
def backup():
    ...
    if xbmcvfs.exists(backup + "/"):
        if not dialog("yesno", "{watchnexus}", translate(33090)):

            return backup()

        delete_folder(backup)

    addon_data = translate_path("special://profile/addon_data/plugin.video.watchnexus")
    destination_data = os.path.join(backup, "addon_data", "plugin.video.watchnexus")
    ...
    db = translate_path(databases["watchnexus"])
    xbmcvfs.copy(db, os.path.join(destination_databases, db.rsplit("\\", 1)[1]))
    LOG.info("copied watchnexus.db")
    ...
    dialog("ok", "{watchnexus}", "%s %s" % (translate(33091), backup))
