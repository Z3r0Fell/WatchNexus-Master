...
def watchnexus_url(client, handler):
...
class API(object):
    ...
    # Bigger section of the WatchNexus api
    ...
    def check_companion_enabled(self):
        ...
        try:
            plugin_settings = (
                self._get("WatchNexus.Plugin.KodiSyncQueue/GetPluginSettings") or {}
            )
    ...
    def get_sync_queue(self, date, filters=None):
        return self._get(
            "WatchNexus.Plugin.KodiSyncQueue/{UserId}/GetItems",
            params={"LastUpdateDT": date, "filter": filters or "None"},
        )

    def get_server_time(self):
        return self._get("WatchNexus.Plugin.KodiSyncQueue/GetServerDateTime")
    ...
    def get_media_segments(self, item_id):
        """Get media segments for an item (WatchNexus 10.10+)."""
        try:
