...
class HTTP(object):
    ...
    def request(self, data, session=None):
        """Give a chance to retry the connection. WatchNexus sometimes can be slow to answer back
            ...
            data["headers"].update(
                {
                    ...
                    or "%s/%s"
                    % (
                        self.config.data.get("app.name", "WatchNexus for Kodi"),
    ...
    def _authorization(self, data):
        ...
        auth = "MediaBrowser "
        auth += "Client=%s, " % quote(
            self.config.data.get("app.name", "WatchNexus for Kodi"), safe=""
        )
        auth += "Device=%s, " % quote(
