...
class Library(threading.Thread):
    ...
    def __init__(self, monitor):
        ...
        self.server = WatchNexus().get_client()
    ...
    def run(self):
        ...
        window("watchnexus_startup.bool", True)
    ...
    def worker_downloads(self):
        """Get items from watchnexus and place them in the appropriate queues."""
        for work_queue in (
    ...
    def worker_sort(self):
        """Get items based on the local watchnexus database and place item in appropriate queues."""
        if self.removed_queue.qsize() and len(self.watchnexus_threads) < 2:
    ...
    def startup(self):
        ...
        try:
            ...
            if settings("SyncInstallRunDone.bool") and settings("kodiCompanion.bool"):
                # None == Unknown
                if self.server.watchnexus.check_companion_enabled() is not False:

                    if not self.fast_sync():
                        dialog("ok", "{watchnexus}", translate(33128))
                ...
                # is False
                    ...
                    dialog("ok", "{watchnexus}", translate(33099))
            ...
            LOG.error(error)
            dialog("ok", "{watchnexus}", translate(33129))
    ...
    def fast_sync(self):
        ...
        for item_id in whitelist:
            library = self.server.watchnexus.get_item(item_id)
        ...
        try:
            ...
            result = self.server.watchnexus.get_sync_queue(
...
class UpdateWorker(threading.Thread):
    ...
    def run(self):
        with self.lock, self.database as kodidb, Database("watchnexus") as watchnexusdb:
            default_args = (self.server, watchnexusdb, kodidb, self.direct_path)
...
class SortWorker(threading.Thread):
    ...
    def run(self):
        ...
        with Database("watchnexus") as watchnexusdb:
            database = watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor)
            ...
            while True:
                ...
                try:
                    ...
                    if media:
                        ...
                        if not items:
                            LOG.info(
                                "Could not find media %s in the watchnexus database.",
                                item_id,
                            )
                        else:
                            for item in items:
