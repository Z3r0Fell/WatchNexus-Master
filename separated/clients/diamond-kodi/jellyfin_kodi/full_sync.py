...
from .database import Database, get_sync, save_sync, watchnexus_db
...
class FullSync(object):
    ...
    i.e. with FullSync('watchnexus') as sync:
        sync.libraries()
    """
    ...
    def __init__(self, library, server):
        ...
        if self.running:
            dialog("ok", "{watchnexus}", translate(33197))
    ...
    def __enter__(self):
        ...
        self.running = True
        window("watchnexus_sync.bool", True)
    ...
    def libraries(self, libraries=None, update=False):
        ...
        if libraries:
            ...
            for library_id in libraries:
                # Look up library in local WatchNexus database
    ...
    def get_libraries(self):
        with Database("watchnexus") as watchnexusdb:
            return watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor).get_views()

    def get_library(self, library_id):
        with Database("watchnexus") as watchnexusdb:
            return watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor).get_view(library_id)
    ...
    def mapping(self):
        ...
        if self.sync["Libraries"]:
            ...
            if not dialog("yesno", "{watchnexus}", translate(33102)):

                if not dialog("yesno", "{watchnexus}", translate(33173)):
                    dialog("ok", "{watchnexus}", translate(33122))
    ...
    def process_library(self, library_id):
        ...
        try:
            ...
            library = self.server.watchnexus.get_item(library_id.replace("Mixed:", ""))
            ...
            dialog("ok", "{watchnexus}", translate(33119))
    ...
    def movies(self, library, dialog):
        ...
        for items in server.get_items(
            ...
            with self.video_database_locks() as (videodb, watchnexusdb):
                obj = Movies(
                    self.server, watchnexusdb, videodb, self.direct_path, library
                )
        ...
        with self.video_database_locks() as (videodb, watchnexusdb):
            obj = Movies(self.server, watchnexusdb, videodb, self.direct_path, library)
            obj.item_ids = processed_ids

            if self.update_library:
                self.movies_compare(library, obj, watchnexusdb)

    def movies_compare(self, library, obj, watchnexusydb):
        """Compare entries from library to what's in the watchnexusdb. Remove surplus"""
        db = watchnexus_db.WatchNexusDatabase(watchnexusydb.cursor)
    ...
    def tvshows(self, library, dialog):
        ...
        for items in server.get_items(
            ...
            with self.video_database_locks() as (videodb, watchnexusdb):
                obj = TVShows(
                    self.server, watchnexusdb, videodb, self.direct_path, library, True
                )
        ...
        with self.video_database_locks() as (videodb, watchnexusdb):
            ...
            obj.item_ids = processed_ids
            if self.update_library:
                self.tvshows_compare(library, obj, watchnexusdb)

    def tvshows_compare(self, library, obj, watchnexusdb):
        """Compare entries from library to what's in the watchnexusdb. Remove surplus"""
    ...
    def musicvideos(self, library, dialog):
        ...
        with self.video_database_locks() as (videodb, watchnexusdb):
            ...
            obj.item_ids = processed_ids
            if self.update_library:
                self.musicvideos_compare(library, obj, watchnexusdb)

    def musicvideos_compare(self, library, obj, watchnexusdb):
        """Compare entries from library to what's in the watchnexusdb. Remove surplus"""
        db = watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor)
    ...
    def music(self, library, dialog):
        ...
        with self.library.music_database_lock:
            with Database("music") as musicdb:
                with Database("watchnexus") as watchnexusdb:
                    obj = Music(
                        self.server, watchnexusdb, musicdb, self.direct_path, library
                    )
                    ...
                    if self.update_library:
                        self.music_compare(library, obj, watchnexusdb)

    def music_compare(self, library, obj, watchnexusdb):
        """Compare entries from library to what's in the watchnexusdb. Remove surplus"""
        db = watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor)
                ...
                with (
                    ...
                    with Database(media) as kodidb:
                        ...
                        if library.media_type == "mixed":
                            ...
                            obj = Movies(
                                self.server, watchnexusdb, kodidb, direct_path, library
                            ).remove
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        """Exiting sync"""
        self.running = False
        window("watchnexus_sync", clear=True)
