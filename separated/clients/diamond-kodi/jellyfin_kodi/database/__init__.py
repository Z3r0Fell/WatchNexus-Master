...
class Database(object):
    ...
    i.e. with Database('watchnexus') as db:
        db.cursor
        db.conn.commit()
    """
    ...
    def __init__(self, db_file=None, commit_close=True):
        """file: watchnexus, texture, music, video, :memory: or path to file"""
    ...
    def __enter__(self):
        ...
        if self.db_file in ("video", "music", "texture", "watchnexus"):
            self.conn.execute(
        ...
        if not window("watchnexus_db_check.bool") and self.db_file == "watchnexus":

            window("watchnexus_db_check.bool", True)
            watchnexus_tables(self.cursor)
            self.conn.commit()
...
def watchnexus_tables(cursor):
    """Create the tables for the watchnexus database.
    watchnexus, view, version
    """
    cursor.execute("""CREATE TABLE IF NOT EXISTS watchnexus(
        watchnexus_id TEXT UNIQUE, media_folder TEXT, watchnexus_type TEXT, media_type TEXT,
        kodi_id INTEGER, kodi_fileid INTEGER, kodi_pathid INTEGER, parent_id INTEGER,
        checksum INTEGER, watchnexus_parent_id TEXT)""")
    cursor.execute("""CREATE TABLE IF NOT EXISTS view(
        view_id TEXT UNIQUE, view_name TEXT, media_type TEXT)""")
    cursor.execute("CREATE TABLE IF NOT EXISTS version(idVersion TEXT)")

    columns = cursor.execute("SELECT * FROM watchnexus")
    if "watchnexus_parent_id" not in [
        description[0] for description in columns.description
    ]:

        LOG.debug("Add missing column watchnexus_parent_id")
        cursor.execute("ALTER TABLE watchnexus ADD COLUMN watchnexus_parent_id 'TEXT'")


def reset():
    """Reset both the watchnexus database and the kodi database."""
    from ..views import Views

    views = Views()

    if not dialog("yesno", "{watchnexus}", translate(33074)):
        return

    window("watchnexus_should_stop.bool", True)
    count = 10

    while window("watchnexus_sync.bool"):

        LOG.info("Sync is running...")
        count -= 1

        if not count:
            dialog("ok", "{watchnexus}", translate(33085))
    ...
    if dialog("yesno", "{watchnexus}", translate(33086)):
        reset_artwork()

    if dialog("yesno", "{watchnexus}", translate(33087)):
    ...
    dialog("ok", "{watchnexus}", translate(33088))
...
def reset_kodi():
    ...
    if settings("enableMusic.bool") or dialog("yesno", "{watchnexus}", translate(33162)):
...
def reset_watchnexus():

    with Database("watchnexus") as watchnexusdb:
        watchnexusdb.cursor.execute(
            "SELECT tbl_name FROM sqlite_master WHERE type='table'"
        )

        for table in watchnexusdb.cursor.fetchall():
            name = table[0]

            if name not in ("version", "view"):
                watchnexusdb.cursor.execute("DELETE FROM " + name)

            watchnexusdb.cursor.execute("DROP table IF EXISTS watchnexus")
            watchnexusdb.cursor.execute("DROP table IF EXISTS view")
            watchnexusdb.cursor.execute("DROP table IF EXISTS version")

    LOG.info("[ reset watchnexus ]")
...
def get_item(kodi_id, media):
    """Get watchnexus item based on kodi id and media."""
    with Database("watchnexus") as watchnexusdb:
        item = watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor).get_full_item_by_kodi_id(
            kodi_id, media
        )

        if not item:
            LOG.debug("Not an watchnexus item")
