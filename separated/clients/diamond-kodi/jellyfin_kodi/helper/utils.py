...
def addon_id():
    return "plugin.video.watchnexus"
...
def event(method, data=None, sender=None, hexlify=False):
    ...
    data = data or {}
    sender = sender or "plugin.video.watchnexus"
...
def dialog(dialog_type, *args, **kwargs):
    ...
    if "heading" in kwargs:
        kwargs["heading"] = kwargs["heading"].replace(
            "{watchnexus}", translate("addon_name")
        )

    if args:
        args = list(args)
        args[0] = args[0].replace("{watchnexus}", translate("addon_name"))
...
def validate(path):
    ...
    if window("watchnexus_pathverified.bool"):
        return True
    ...
    window("watchnexus_pathverified.bool", True)
...
def set_addon_mode():
    ...
    if value:
        dialog("ok", "{watchnexus}", translate(33145))
        path_replacements()
...
class JsonDebugPrinter(object):
    ...
    See: https://github.com/watchnexus/watchnexus-kodi/pull/193
    """
...
def find_library(server, item):
    ...
    ancestors = server.watchnexus.get_ancestors(item["Id"])
