...
__addon__ = xbmcaddon.Addon(id="plugin.video.watchnexus")
        ...
        if self._get_log_level(record.levelno):
            ...
            if self.mask_info:
                for server in self.sensitive["Server"]:
                    string = string.replace(server or "{server}", "{watchnexus-server}")

                for token in self.sensitive["Token"]:
                    string = string.replace(token or "{token}", "{watchnexus-token}")
