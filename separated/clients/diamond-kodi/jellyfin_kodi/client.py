...
def get_device_id(reset=False):
    ...
    It will load from watchnexus_guid file. If it's a fresh
    setup, it will generate a new GUID to uniquely
    identify the setup for all users.

    window prop: watchnexus_deviceId
    """
    client_id = window("watchnexus_deviceId")

    if client_id:
        return client_id

    directory = translate_path("special://profile/addon_data/plugin.video.watchnexus/")

    if not xbmcvfs.exists(directory):
        xbmcvfs.mkdir(directory)

    watchnexus_guid = os.path.join(directory, "watchnexus_guid")
    file_guid = xbmcvfs.File(watchnexus_guid)
    ...
    if not client_id or reset:
        ...
        file_guid = xbmcvfs.File(watchnexus_guid, "w")
        file_guid.write(client_id)

    file_guid.close()
    LOG.debug("DeviceId loaded: %s", client_id)
    window("watchnexus_deviceId", value=client_id)
...
def reset_device_id():
    ...
    window("watchnexus_deviceId", clear=True)
    get_device_id(True)
    dialog("ok", "{watchnexus}", translate(33033))
