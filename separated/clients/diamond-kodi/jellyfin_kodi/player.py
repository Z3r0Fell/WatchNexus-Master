...
class Player(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        ...
        First, ensure previous playback terminated correctly in WatchNexus.
        """
        self.stop_playback()
        self.up_next = False
        count = 0
        ...
        items = window("watchnexus_play.json")
        item = None
        ...
        while not items:
            ...
            if count == 20:
                LOG.info("Could not find watchnexus prop...")
        ...
        window("watchnexus_play.json", items)
        ...
        try:
            item["Server"].watchnexus.session_playing(data)
        except Exception as e:
            LOG.warning("Failed to report session playing: %s", e)
        window("watchnexus.skip.%s.bool" % item["Id"], True)
        ...
        item.update(
            {
                ...
                "Muted": muted,
                "Volume": volume,
                "Server": WatchNexus(item["ServerId"]).get_client(),
                "Paused": False,
            }
        )
    ...
    def next_up(self):
        ...
        next_items = item["Server"].watchnexus.get_adjacent_episodes(
    ...
    def report_playback(self, report=True):
        """Report playback progress to watchnexus server.
        ...
        if window("watchnexus.external.bool"):
            return
        ...
        item["Server"].watchnexus.session_progress(data)

    def onPlayBackStopped(self):
        """Will be called when user stops playing a file."""
        window("watchnexus_play", clear=True)
        self.stop_playback()
        LOG.info("--<[ playback ]")
    ...
    def stop_playback(self):
        ...
        for file in self.played:
            ...
            if window("watchnexus.external.bool"):
                window("watchnexus.external", clear=True)
            ...
            item["Server"].watchnexus.session_stop(data)

            if item.get("LiveStreamId"):

                LOG.info("<[ livestream/%s ]", item["LiveStreamId"])
                item["Server"].watchnexus.close_live_stream(item["LiveStreamId"])

            elif item["PlayMethod"] == "Transcode":

                LOG.info("<[ transcode/%s ]", item["Id"])
                item["Server"].watchnexus.close_transcode(
            ...
            result = item["Server"].watchnexus.get_item(item["Id"]) or {}
            ...
            if "UserData" in result and result["UserData"]["Played"]:
                ...
                if delete:
                    ...
                    if dialog(
                        ...
                        item["Server"].watchnexus.delete_item(item["Id"])

            window("watchnexus.external_check", clear=True)
    ...
    def _fetch_skip_segments(self, item):
        ...
        segments = item["Server"].watchnexus.get_media_segments(item_id)
        if segments:
    ...
    def _show_skip_button(self, segment_type, duration, end_time):
        ...
        try:
            ...
            addon_path = xbmcaddon.Addon("plugin.video.watchnexus").getAddonInfo("path")
            LOG.debug("_show_skip_button: addon_path=%s", addon_path)

            self.skip_dialog = SkipDialog(
                "script-watchnexus-skip.xml",
                addon_path,
                "default",
                "1080i",
            )
