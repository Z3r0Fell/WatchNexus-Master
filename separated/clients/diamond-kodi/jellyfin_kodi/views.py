...
from .database import Database, watchnexus_db, get_sync, save_sync
...
class Views(object):
    ...
    def __init__(self):
        ...
        self.sync = get_sync()
        self.server = WatchNexus()

    def add_library(self, view):
        """Add entry to view table in watchnexus database."""
        with Database("watchnexus") as watchnexusdb:
            watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor).add_view(
                view["Id"], view["Name"], view["Media"]
            )

    def remove_library(self, view_id):
        """Remove entry from view table in watchnexus database."""
        with Database("watchnexus") as watchnexusdb:
            watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor).remove_view(view_id)
    ...
    def get_libraries(self):
        ...
        try:
            libraries = self.server.watchnexus.get_media_folders()["Items"]
            library_ids = [x["Id"] for x in libraries]
            for view in self.server.watchnexus.get_views()["Items"]:
    ...
    def get_views(self):
        ...
        with Database("watchnexus") as watchnexusdb:
            ...
            views = watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor).get_views()
            removed = []
    ...
    def add_playlist(self, path, view, mixed=False):
        ...
        file = os.path.join(path, "watchnexus%s%s.xsp" % (view["Media"], view["Id"]))
    ...
    def add_nodes(self, path, view, mixed=False):
        ...
        folder = os.path.join(path, "watchnexus%s%s" % (view["Media"], view["Id"]))
    ...
    def add_single_node(self, path, index, item_type, view):
        ...
        file = os.path.join(path, "watchnexus_%s.xml" % view["Tag"].replace(" ", ""))
    ...
    def node_root(self, root, index):
        ...
        etree.SubElement(element, "icon").text = (
            "special://home/addons/plugin.video.watchnexus/resources/icon.png"
        )
    ...
    def add_dynamic_node(self, index, file, view, node, name, path):
        ...
        # Migration for https://github.com/watchnexus/watchnexus-kodi/issues/239
    ...
    def order_media_folders(self, folders):
        """Returns a list of sorted media folders based on the WatchNexus views.
    ...
    def window_nodes(self):
        ...
        Set up the window properties that reflect the watchnexus server views and more.
        """
        self.window_clear()
        self.window_clear("WatchNexus.wnodes")

        with Database("watchnexus") as watchnexusdb:
            libraries = watchnexus_db.WatchNexusDatabase(watchnexusdb.cursor).get_views()
        ...
        window("WatchNexus.nodes.total", str(index))
        window("WatchNexus.wnodes.total", str(windex))
    ...
    def window_node(self, index, view, node=None, node_label=None):
        ...
        if node in ("all", "music"):
            ...
            window_prop = "WatchNexus.nodes.%s" % index
            ...
            window_prop = "WatchNexus.nodes.%s.%s" % (index, node)
    ...
    def window_single_node(self, index, item_type, view):
        ...
        path = "library://video/watchnexus_%s.xml" % view["Tag"].replace(" ", "")
        window_path = "ActivateWindow(Videos,%s,return)" % path
    ...
    def window_wnode(self, index, view, node=None, node_label=None):
        ...
        if node == "all":
            ...
            window_prop = "WatchNexus.wnodes.%s" % index
            window("%s.index" % window_prop, path.replace("all.xml", ""))  # dir
            window("%s.title" % window_prop, view["Name"])
        elif node == "browse":
            ...
            window_prop = "WatchNexus.wnodes.%s.%s" % (index, node)
            window("%s.title" % window_prop, node_label)
        window("%s.content" % window_prop, path)
    ...
    def window_path(self, view, node):
        return "library://video/watchnexus%s%s/%s.xml" % (view["Media"], view["Id"], node)
    ...
    def window_nextepisodes(self, view):
        ...
        return "%s?%s" % ("plugin://plugin.video.watchnexus/", urlencode(params))
    ...
    def window_clear(self, name=None):
        ...
        total = int(window((name or "WatchNexus.nodes") + ".total") or 0)
        props = [
            "index",
            "id",
            "path",
        ...
        for i in range(total):
            for prop in props:
                window("WatchNexus.nodes.%s.%s" % (str(i), prop), clear=True)

        for prop in props:
            window("WatchNexus.nodes.%s" % prop, clear=True)
    ...
    def delete_playlists(self):
        """Remove all watchnexus playlists."""
        path = translate_path("special://profile/playlists/video/")
        _, files = xbmcvfs.listdir(path)
        for file in files:
            if file.startswith("watchnexus"):
    ...
    def delete_playlist_by_id(self, view_id):
        ...
        for file in files:
            ...
            if file.startswith("watchnexus") and file.endswith("%s.xsp" % view_id):
                self.delete_playlist(os.path.join(path, file))
    ...
    def delete_nodes(self):
        ...
        for directory in dirs:
            ...
            if directory.startswith("watchnexus"):
    ...
    def delete_node_by_id(self, view_id):
        ...
        for directory in dirs:
            ...
            if directory.startswith("watchnexus") and directory.endswith(view_id):
                _, files = xbmcvfs.listdir(os.path.join(path, directory))
