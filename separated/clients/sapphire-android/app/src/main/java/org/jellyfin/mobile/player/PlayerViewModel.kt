...
import org.jellyfin.mobile.player.source.WatchNexusMediaSource
import org.jellyfin.mobile.player.source.RemoteWatchNexusMediaSource
...
class PlayerViewModel(application: Application) : AndroidViewModel(application), KoinComponent, Player.Listener {
    ...
    val mediaSourceOrNull: WatchNexusMediaSource?
    ...
    fun load(jellyfinMediaSource: WatchNexusMediaSource, exoMediaSource: MediaSource, playWhenReady: Boolean) {
        ...
        if (jellyfinMediaSource is RemoteWatchNexusMediaSource) {
            viewModelScope.launch {
    ...
    private fun startProgressUpdates() {
        if (mediaSourceOrNull != null && mediaSourceOrNull !is RemoteWatchNexusMediaSource) return
    ...
    private suspend fun Player.reportPlaybackStart(mediaSource: RemoteWatchNexusMediaSource) {
        try {
    ...
    private suspend fun Player.reportPlaybackState() {
        val mediaSource = mediaSourceOrNull as? RemoteWatchNexusMediaSource ?: return
        val playbackPosition = currentPosition.milliseconds
    ...
    suspend fun stopTranscoding(mediaSource: RemoteWatchNexusMediaSource) {
    ...
    private fun applyMediaSegments(jellyfinMediaSource: WatchNexusMediaSource) {
