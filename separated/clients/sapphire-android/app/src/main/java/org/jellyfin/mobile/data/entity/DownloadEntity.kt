...
import org.jellyfin.mobile.player.source.LocalWatchNexusMediaSource
...
@TypeConverters(LocalWatchNexusMediaSourceConverter::class)
data class DownloadEntity(
    @PrimaryKey
    @ColumnInfo(name = ITEM_ID)
    val itemId: String,
    @ColumnInfo(name = MEDIA_SOURCE)
    val mediaSource: LocalWatchNexusMediaSource,
) {
    /**
     * Converts the [mediaSource] string to a [LocalWatchNexusMediaSource] object.
     *
    ...
    ): LocalWatchNexusMediaSource = mediaSource
        .also { localWatchNexusMediaSource ->
            startTime
                ?.let { localWatchNexusMediaSource.startTime = it }
            audioStreamIndex
                ?.let { localWatchNexusMediaSource.mediaStreams[it] }
                ?.let(localWatchNexusMediaSource::selectAudioStream)
            subtitleStreamIndex
                ?.run {
                    takeUnless { it == -1 }
                        ?.let { localWatchNexusMediaSource.mediaStreams[it] }
                        ?: localWatchNexusMediaSource.selectSubtitleStream(null)
                }
        }

    constructor(mediaSource: LocalWatchNexusMediaSource) :
...
class LocalWatchNexusMediaSourceConverter {
    @TypeConverter
    fun toLocalWatchNexusMediaSource(value: String): LocalWatchNexusMediaSource = decodeFromString(value)

    @TypeConverter
    fun fromLocalWatchNexusMediaSource(value: LocalWatchNexusMediaSource): String = Json.encodeToString(value)
}
